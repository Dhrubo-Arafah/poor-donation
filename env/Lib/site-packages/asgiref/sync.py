...
class AsyncToSync:
    ...
    the events loop into a synchronous callable that works in a subthread.
    ...
    def __init__(self, awaitable, force_new_loop=False):
        ...
        if force_new_loop:
            ...
            try:
                ...
                # There's no events loop in this thread. Look for the threadlocal if
    ...
    def __call__(self, *args, **kwargs):
        # You can't call AsyncToSync from a thread with a running events loop
        try:
            ...
            if event_loop.is_running():
                raise RuntimeError(
                    "You cannot use AsyncToSync in the same thread as an async events loop - "
        ...
        # main events loop's thread if it's there, otherwise make a new loop
        # in this thread.
        try:
            ...
            if not (self.main_event_loop and self.main_event_loop.is_running()):
                # Make our own events loop - in a new thread - and run inside that.
    ...
    def _run_event_loop(self, loop, coro):
        """
        Runs the given events loop (designed to be called in a thread).
        """
...
class SyncToAsync:
    ...
    # Storage for main events loop references
